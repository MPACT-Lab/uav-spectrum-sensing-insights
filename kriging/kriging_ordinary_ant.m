...
clear;
close all;
% pre requisite threed_corr_gen_ok_sk_ant.m (without normal transformation)
addpath('functions\')
...
num_known_samples = [50, 150, 250, 500]; % per height
num_known_samples = [50, 75, 100, 125, 150, 175, 200, 225, 250]; % per height
radius_all = [ 70, 200];

radius_colors = ["r", "g", "b", "c", "m", "y", "k"];
num_test_points = 25; % per height
threed = 1; % exp(-y) improves performance, although it makes variance bigger (no one close to 0)
...
for repeat_id = 1:num_repeat
    ...
    for height_id = 1:length(all_heights)
        ...
        load(sprintf("../corr_prof/profiles/correlation_table_3D_a2g_smooth_%dm.mat",all_heights_train(height_id))) % 'hor_dist_list','ver_dist_list', 'corr_list', 'mu_sha', "var_sha", 'd_int', 'h_int', 'correlation_f'
        ...
        for meas_i = 1:length(num_known_samples)
            ...
            for radi_i = 1:length(radius_all)
               ...
               [est_tar, est_variance, num_points] = my_oridinary_kriging(meas_correlation_all,...
...
%% 
save results/res_kriging_ordinary_sung_joon_50_250_3_a2g_5k.mat error_all_2d num_meas_used_2d lat_calc lon_calc h_calc radius_all num_known_samples variance_all_2d all_heights threed