...
clear;
close all;
% pre requisite threed_corr_gen_trans_ok_sk.m (without normal transformation)
addpath('functions\')
...
for repeat_id = 1:num_repeat
    ...
    for height_id = 1:length(all_heights)
        load(sprintf("../corr_prof/profiles/correlation_table_3D_gauss_%dm.mat", all_heights_train(height_id))) % 'hor_dist_list','ver_dist_list', 'corr_list', 'mu_sha', "var_sha", 'd_int', 'h_int', 'correlation_f'
        load(sprintf("../corr_prof/profiles/normal_transformation_%dm.mat", all_heights_train(height_id))) % trans_gauss trans_raw second_der_value
        ...
        for meas_i = 1:length(num_known_samples)
            ...
            for radi_i = 1:length(radius_all)
               ...
               [est_tar, est_tar_biased, est_variance, num_points] = my_simple_kriging_new_eqn(meas_correlation_all,...
    target_cross_correlation_all, index_valid_radi, sha_meas, var_sha, mu_sha, second_der_value, trans_raw);
               error_all_2d_biased(repeat_id, height_id, meas_i, radi_i, :) = est_tar_biased - sha_tar_raw;
               error_all_2d(repeat_id, height_id, meas_i, radi_i, :) = est_tar - sha_tar_raw;
...
%% 
save results/res_kriging_simple_gaussian_sung_joon_50_250_2_5k.mat error_all_2d error_all_2d_biased num_meas_used_2d lat_calc lon_calc h_calc radius_all num_known_samples variance_all_2d all_heights