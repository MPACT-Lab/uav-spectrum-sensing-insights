function [NewMAT, ier] = MatrixCompletionErrorWeighted(A, Zone,N, Mode, lambda_tol, tol, display, Weights)
% Inputs:
...
% Weights - inverse of estimated variance
%
% Output:
...
while ((err > tol) || (abs(lambda-lambda_prev) > lambda_tol))
    ...
    if strcmp(Mode,'nuclear')
        ...
        [NewMAT, Error] = MatApproxNuclearErrorWeighted(A.*Zone, NewMAT, Zone, lambda, N, tol, Weights, display); % Was A.*Zone instead NewMAT in the input
    elseif strcmp(Mode,'spectral');
