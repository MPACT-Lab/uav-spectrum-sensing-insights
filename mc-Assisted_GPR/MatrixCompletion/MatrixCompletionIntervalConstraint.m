function [NewMAT, ier] = MatrixCompletionIntervalConstraint(A, Zone,N, Mode, lambda_tol, tol, display, Weights)
% Inputs:
...
Converge=0;
%NewMAT=A.*Zone;
while (any(any(err > tol)) || (abs(lambda-lambda_prev) > lambda_tol))
    Counter=Counter+1;
    lambda_prev = lambda;
    ...
    if strcmp(Mode,'nuclear')
        ...
        [NewMAT, err] = MatApproxNuclearIntervalConstraint(A.*Zone, NewMAT, Zone, lambda, N, tol, display);
