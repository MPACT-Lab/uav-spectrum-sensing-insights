function [NewMAT, ier] = MatrixCompletionIntervalMatrix(A, Zone,N, Mode, lambda_tol, std_dev_tol, display)
% Inputs:
...
% tol - interval tolerance (matrix, all values positive) A(zone) +-
% interval(zone)
...
Converge=0;
%NewMAT=A.*Zone;
while (any(any(err > std_dev_tol)) || (abs(lambda-lambda_prev) > lambda_tol))
    Counter=Counter+1;
    lambda_prev = lambda;
    lambda=(min_lambda+max_lambda)/2;
    if strcmp(Mode,'nuclear')
        if(display)
            fprintf('nuclear norm before operation: %g \n',sum(svd(NewMAT)));
        end
        [NewMAT, err] = MatApproxNuclearIntervalMatrix(A.*Zone, NewMAT, Zone, lambda, N, std_dev_tol, display);
        %fprintf('nuclear norm after operation: %g \n',sum(svd(NewMAT)));
    elseif strcmp(Mode,'spectral');
        [NewMAT, err] = MatApproxSpectral(A.*Zone, NewMAT, Zone, lambda, N, tol);
    elseif strcmp(Mode, 'NuclearWeighted')
        [NewMAT, err] = MatApproxNuclearWeighted(A.*Zone, NewMAT, Zone, Weights, lambda, N, tol, display);
    else
        fprintf('Undefined mode. quitting. \n');
        return;
    end
    if(display)
        fprintf('Obtained nuclear norm: %g \n',sum(svd(NewMAT)));
    end
    if any(any(err > std_dev_tol))
        min_lambda=lambda;
    else
        max_lambda=lambda;
    end
...
if display
    fprintf('Max Error=%g, lambda=%g, lambda-lambda_prev=%g, Counter=%d \n',max2(err-std_dev_tol),lambda, lambda-lambda_prev,Counter);
end
