...
all_heights = [30]; % change it {30, 50, 70, 90, 110}
exp_name = "_" + num2str(all_heights(1))+"m";
...
selec_ids_indx = randi([1 length(sha_all)],1,num_test_points);
sha_selected = sha_all(selec_ids_indx);

histogram(sha_selected)
xlabel('Shadow fading')
ylabel('Frequency')
set(gca,'FontSize',12)
...
if h
    fprintf('The data significantly deviates from a normal distribution.\n');
else
    fprintf('The data does not significantly deviate from a normal distribution.\n');
end

%% converting to norm distribution

[sha_raw_cdf, sha_raw] = ecdf(sha_all);
sha_standard_normal = norminv(sha_raw_cdf);
sha_standard_normal(sha_standard_normal>4.25) = 4.25;
sha_standard_normal(sha_standard_normal<-4.25) = -4.25;
trans_gauss=fit(sha_raw,sha_standard_normal,'poly3')
figure(5)
plot(trans_gauss, sha_raw, sha_standard_normal)
xlabel('Shadow fading')
ylabel('Transformed variable')
set(gca,'FontSize',12)
% Get the handle of the legend
hLegend = legend;
% Set the font size of the legend
fontSize = 12; % Change this to the desired font size
set(hLegend, 'FontSize', fontSize);
sha_standard_normal_approx = trans_gauss(sha_all);
sha_trns_mean = mean(sha_standard_normal_approx);
figure(6)
histogram(sha_standard_normal_approx,'Normalization','pdf')
xlabel('Transformed variable')
ylabel('Frequency')
set(gca,'FontSize',12)
syms sha_tr % sha_transformed
p = [trans_gauss.p1 trans_gauss.p2 trans_gauss.p3 trans_gauss.p4-sha_tr];
s = roots(p);
g  = s(1); % imag(subs(g))=0 when sha_tr=2
trans_raw = matlabFunction(g);
trans_raw_der2 = diff(diff(g));
sha_tr = sha_trns_mean;
second_der_value = double(subs(trans_raw_der2));
save(sprintf("profiles/normal_transformation"+exp_name+".mat"), 'trans_gauss', 'trans_raw', 'second_der_value')
sha_all =  sha_standard_normal_approx;
%% 
...
save(sprintf("profiles/correlation_table_3D_gauss"+exp_name+".mat"),'hor_dist_list','ver_dist_list', 'corr_list', 'mu_sha', "var_sha", 'd_int', 'h_int', 'correlation_f')
%% 
