...
for i = 1:length(all_heights)
    load(sprintf('../data_gen/LTE_dataset/processed_data/filtererd_data_with_dist_ori_%d_meas_rad.mat',all_heights(i)))
...
for height_id = 1:length(all_heights)
    ...
    load(sprintf("../corr_prof/profiles/correlation_table_3D_meas_%dm_rad.mat", all_heights_train(height_id))) % 'hor_dist_list','ver_dist_list', 'corr_list', 'mu_sha', "var_sha", 'd_int', 'h_int', 'correlation_f'
    %load("normal_transformation_meas.mat") % trans_gauss trans_raw second_der_value
    ...
    for repeat_id = 1:num_repeat
        ...
        for meas_i = 1:length(num_known_samples)
            ...
            for radi_i = 1:length(radius_all)
               ...
               [est_tar, num_points] = my_simple_kriging(meas_correlation_all,...
    target_cross_correlation_all, index_valid_radi, sha_meas, var_sha, mu_sha);
...
%% 
save results\res_sk_lte_70m.mat error_all_2d num_meas_used_2d lat_calc lon_calc h_calc radius_all num_known_samples all_heights threed
