function [rmse_meas_radius_2d, rmse_meas_radius_3d, valid_est1, valid_est2] = calc_rmse_mean_sung_joon(error_all_2d,error_all_3d)
    ...
    for meas_i = 1:size(error_all_2d,2)
        for radi_i = 1:size(error_all_2d,3)
            ...
            if length(rmse_samples) < 1
                ...
                rmse_meas_radius_2d(meas_i,radi_i) = mean(rmse_samples);
            end
        end
    end
    ...
    for meas_i = 1:size(error_all_3d,2)
        for radi_i = 1:size(error_all_3d,3)
            ...
            if length(rmse_samples) < 1
                ...
                rmse_meas_radius_3d(meas_i,radi_i) = mean(rmse_samples);
            end
        end
    end
